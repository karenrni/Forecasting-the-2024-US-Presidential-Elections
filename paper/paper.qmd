...
# Define swing states and highlight in state_predictions
...
# Plot with threshold line at 0.5
ggplot(state_predictions, aes(x = reorder(state, avg_predicted_prob_harris), y = avg_predicted_prob_harris, fill = swing_state)) +
  ...
  labs(
    title = "Predicted Probability of Harris Winning by Swing State",
    x = "State",
    y = "Predicted Probability for Harris",
    fill = "State Type"
  ) +
...
ggplot(merged_data, aes(x = long, y = lat, group = group, fill = avg_predicted_prob_harris)) +
  geom_polygon(color = "black", size = 0.3) + 
  scale_fill_gradient(low = "lightblue", high = "darkblue", na.value = "darkgrey", name = "Predicted Support") +
  coord_fixed(1.3) + 
  labs(
    title = "Predicted Support for Harris by State",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
