...
```{r}
#| label: fig-candidate
#| fig-cap: Distribution of Polls by Harris vs Trump
#| echo: false
#| warning: false
#| fig-align: center
#| fig-width: 5
...
#| fig-cap: Distribution of Candidate Preference by State
#| echo: false
#| warning: false
#| fig-align: center
#| fig-width: 5
...
ggplot(clean_president_polls, aes(x = state, fill = factor(candidate_chosen, labels = c("Trump", "Harris")))) +
  ...
  labs(x = "State", y = "Proportion", fill = "Chosen Candidate") +
  scale_fill_manual(values = c("red", "blue")) +
  theme_classic() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, margin = margin(t = 5), size = 6),
    axis.title.x = element_text(size = 8),
    axis.title.y = element_text(size = 8),
    legend.text = element_text(size = 6), 
    legend.title = element_text(size = 6)
...
#| fig-cap: Distribution of Candidate Support by Chosen Candidate
#| echo: false
#| warning: false
#| fig-align: center
#| fig-width: 5
...
ggplot(clean_president_polls, aes(x = pct, fill = candidate_name)) +
  ...
  labs(x = "Percentage Support", y = "Density") +
...
Complementing this, [@fig-pollsters] shows the distribution of polls by individual pollsters, including only those with more than 20 counts. In this graph, bar colors indicate each pollster's Numeric Grade, with darker shades of blue representing higher reliability scores. Notably, pollsters such as Siena/NYT and Marquette Law School have the highest Numeric Grade of 3.0, marking them as highly reliable sources. In contrast, pollsters like Trafalgar Group and Patriot Polling, with scores of 0.7 and 1.1, respectively, are represented as lower-quality sources.
...
ggplot(clean_president_polls, aes(x = numeric_grade)) +
  ...
  labs(x = "Numeric Grade", y = "Frequency") +
  theme_classic() +
  theme(
    axis.title.x = element_text(size = 8),
    axis.title.y = element_text(size = 8)
...
```{r}
#| label: fig-pollsters
#| fig-cap: Distribution of Pollsters by Numeric Grade 
#| echo: false
#| warning: false
#| fig-align: center
#| fig-width: 5
...
ggplot(pollsters_over_20, aes(x = reorder(pollster, -num_polls), y = num_polls)) +
  ...
  labs(x = "Pollster", y = "Number of Polls") +
...
#| fig-cap: Number of Pollsters by State
#| echo: false
#| warning: false
#| fig-align: center
#| fig-width: 5
...
```{r}
#| label: fig-support
#| fig-cap: Candidate with Highest Support Percentage by State
#| echo: false
#| warning: false
#| fig-align: center
#| fig-width: 5
...
* Percentage Support (Pct): This variable is integral to the model as it captures the nuanced impact of varying levels of support for the candidates. By utilizing percentage support as a continuous measure, the model facilitates a more detailed examination of how incremental changes in support influence the likelihood of selecting Harris over Trump. This approach enhances the understanding of voter behavior compared to categorizing support into discrete groups, which could obscure critical trends and insights. 

* Random Effects: Treating pollster and state effects as random variables accounts for unobserved heterogeneity in the data. Different pollsters may use varied methodologies, resulting in fluctuations in reported support levels that fixed effects alone cannot capture. Additionally, geographical differences can influence voter preferences, highlighting the need to model state-level variability. By incorporating random effects for both pollster and state, the model effectively addresses this variability, enhancing its robustness and validity. This approach recognizes that the factors influencing voter support can differ significantly depending on the polling context.
...
This prior is centered at 0.5 with a standard deviation of 0.1, reflecting the belief that, in the absence of strong data, the probabilities of support for both candidates are expected to be balanced. This choice of prior is particularly advantageous in situations where data is limited or when there is uncertainty regarding the expected distribution of voter support. By employing a normal prior, we establish a baseline assumption that allows the model to remain flexible and effectively integrate new data as it becomes available.
...
Although the Bayesian model still shows room for improvement—evidenced by F1 Score and AUC values not close to 1 and a RMSE indicating some prediction error—it outperforms the logistic model across all metrics. This suggests that the approach, which incorporates random effects to account for variability from different pollsters and states, offers a more nuanced understanding of voter support.

### Diagnostics
To validate the Bayesian framework, several diagnostic checks were conducted to assess robustness and ensure that underlying assumptions were satisfied. Key among these were convergence assessments and posterior predictive checks. These checks provided a visual representation of how well the predictions align with the observed data. The graph reveals a close overlap between the observed outcomes \(y\) and the replicated predictions ($y_{\text{rep}}$), demonstrating effective capture of the distribution of voter support for Harris. This strong alignment suggests that the approach not only fits the training data well but also generalizes effectively to new observations. The flexibility of this framework allows for the incorporation of prior information and consideration of variability due to pollster and state effects, enhancing overall robustness (Refer to the @sec-app-pp, [@tbl-pp] for a visual illustration).
...
The analysis of the national predicted popular vote percentages indicates that Kamala Harris is projected to receive 50.44% of the vote, while Donald Trump is expected to garner 49.56%. These figures, illustrated in [@fig-national-vote] and calculated by averaging the mean predictions across all states, reveal a narrow lead of just 0.88% for Harris, highlighting a highly competitive race. This reveals a lack of dominance in candidacy and general uncertainty of pollster outcomes, suggesting that even this slight lead may not be substantial.
```{r}
...
Additionally, [@fig-heatmap] presents a heat map of Harris's predicted probability by state, illustrating her lead over the majority of states but revealing that this advantage is generally weak, with most not peaking over a 1% advantage. Most states show only a slight preference for Harris over Trump, with the margin of error spanning both candidates in many cases. Specifically, out of the 43 states with available data, 25 states lean toward Harris, yet none of these states shows a substantial lead over more than a few percent over Trump. This pattern additionally shows the substantial closeness of the race, as evidenced by the margin of error, where overlapping intervals highlight the slim advantage and limited certainty in Harris's predicted support.
```{r}
#| label: fig-heatmap
#| fig-cap: "Heatmap of Predicted Support for Harris by State"
#| echo: false
#| warning: false
#| fig-align: center
#| fig-width: 5
...
# Ensure `state_predictions` has the necessary data
state_predictions <- clean_president_polls %>%
  group_by(state) %>%
...
# Convert state names to lowercase for matching with `maps` package data
state_predictions$state <- tolower(state_predictions$state)

# Load USA state map data and merge with predictions
usa_states <- map_data("state")
merged_data <- merge(usa_states, state_predictions, by.x = "region", by.y = "state", sort = FALSE, all.x = TRUE)
merged_data <- merged_data[order(merged_data$group, merged_data$order), ]

# Plot heatmap for Harris's support by state, with NA values as dark grey
ggplot(merged_data, aes(x = long, y = lat, group = group, fill = avg_predicted_prob_harris)) +
  geom_polygon(color = "black", size = 0.3) + 
  scale_fill_gradient(low = "lightblue", high = "blue", na.value = "darkgrey", name = "Predicted Support") +
  coord_fixed(1.3) + 
  labs(
    x = NULL,
    y = NULL
...
ggplot(state_predictions, aes(x = reorder(state, avg_predicted_prob_harris), y = avg_predicted_prob_harris, fill = swing_state)) +
  ...
  annotate("text", x = 1, y = 0.51, label = "0.5", color = "red", hjust = 0, size = 4) +
  theme_classic() +
  theme(
...
# Create a custom data frame with the results
custom_summary <- data.frame(
  Parameter = c("Intercept", 
                "Percentage Support", 
                "Pollster Variability", 
                "State Variability", 
...
# Methodology Analysis of The Washington Post Polling {#sec-meth-analysis}
...
# Idealized Survey & Methodology - $100K Budget {#sec-ideal-surv}
...
# Posterior Predictive Check {#sec-app-pp}
* [@tbl-pp]
```{r}
#| include: false
#| warning: false
...
```{r}
#| label: tbl-pp
#| tbl-cap: Posterior Predictive Check for Model Fit Assessment
#| echo: false
#| warning: false
#| tbl-align: center
